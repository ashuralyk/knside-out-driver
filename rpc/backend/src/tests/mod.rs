...
use ko_protocol::traits::{Backend, Driver};
...
//     let deployment = mol_deployment(lua_code.as_bytes());
...
fn sign_and_push(tx: TransactionView) {
    // sign transaction
    let privkey = SecretKey::from_slice(OWNER_PRIVATE_KEY.as_bytes()).unwrap();
    let mut driver = DriverImpl::new(CKB_URL, &privkey);
    let signature = driver.sign_ko_transaction(&tx);
...
async fn deploy_project_deployment_cell() {
    ...
    // create digest
    let mut backend = BackendImpl::new(CKB_URL, CKB_INDEXER_URL);
